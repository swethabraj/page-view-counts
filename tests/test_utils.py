...
def test_read_stream_pass(spark):
    read_stream_df = read_stream(schema, spark, input_format="json", input_path=f"{curr_dir}/tests/data/source", read_options={"mutliline": "true"})
...
def test_write_stream(spark):
    ...
    read_stream_df = read_stream(schema, spark, input_format="json", input_path= f"{curr_dir}/tests/data/source", read_options={"multiline": "true"})
    query = write_stream(read_stream_df, output_sink="memory")
    
    actual_output = spark.table("query_result")
    query.awaitTermination()
    
    expected_output = spark.read.format("json").schema(schema).option("multiline", "true").load(f"{curr_dir}/tests/data/source")
